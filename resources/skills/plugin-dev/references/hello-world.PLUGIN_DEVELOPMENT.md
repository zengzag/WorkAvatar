...
> 面向外部开发者：独立开发、打包、分发 WorkAvatar 插件。协议细节用 read_reference 读取 `API_REFERENCE.md`，能力矩阵见 `CAPABILITY_MATRIX.md`，完整协议见 `PROTOCOL.md`（均为本技能 references）；也可在 WorkAvatar 内通过内置 plugin-dev skill 获取全部资料并由数字员工代为脚手架。
