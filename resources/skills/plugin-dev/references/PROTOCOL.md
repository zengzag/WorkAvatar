...
插件变更（启停 / 导入 / 删除 / 升级）**即时生效，无需重启**：宿主只对受影响插件做增量激活/下线，渲染端经 `plugin:changed` 广播增量加载/卸载（免整页刷新，不打断正在进行的智能体生成）。升级已加载插件时渲染端动态 import 带版本参数防模块缓存。手动放入插件目录后可在插件管理页点击「刷新」触发重新扫描。
