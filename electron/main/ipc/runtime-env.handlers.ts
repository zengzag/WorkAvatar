...
import type { RuntimeEnvInstallParams } from '../../shared/ipc-channels'
import RuntimeEnvService from '../services/runtime-env.service'
import { safeHandle } from './_shared'
import { createLogger } from '../services/logger'

const logger = createLogger('RuntimeEnv-Handler')

/**
 * 运行时环境检测 IPC handlers。
 *
 * 暴露三个 invoke 通道（list / install / cancel-install），
 * 安装进度通过 RUNTIME_ENV_PROGRESS 事件通道主动推送至渲染进程。
 */
export function registerRuntimeEnvHandlers(): void {
  const service = RuntimeEnvService.getInstance()

  // 列出所有受支持运行时的检测状态
  safeHandle(IPC_CHANNELS.RUNTIME_ENV_LIST, async () => {
  ...
  ipcMain.handle(IPC_CHANNELS.RUNTIME_ENV_INSTALL, async (_event, params: RuntimeEnvInstallParams) => {
    try {
      if (!params?.toolId) {
      ...
      logger.error(`IPC handler error [RUNTIME_ENV_INSTALL]:`, msg)
  ...
  // 取消正在进行的安装
  safeHandle(IPC_CHANNELS.RUNTIME_ENV_CANCEL_INSTALL, async () => {
