...
/** 单向同步引擎：本地 SQLite → Outlook（Graph API） */
class OutlookSyncService extends ScheduledTaskBase {
  private static instance: OutlookSyncService
  private db: any
  ...
  private constructor() {
    super('OutlookSync', 60_000)
    this.db = DatabaseService.getInstance().getDb()
  }

  static getInstance(): OutlookSyncService {
    if (!OutlookSyncService.instance) {
      OutlookSyncService.instance = new OutlookSyncService()
    }
    return OutlookSyncService.instance
  }
  ...
  getConfig(): OutlookSyncConfig {
    try {
      const row = this.db.prepare('SELECT value FROM settings WHERE key = ?').get(CONFIG_KEY) as { value?: string } | undefined
  ...
  private loadState(): { last_result: OutlookSyncResult | null; last_error: string | null } {
    try {
      const row = this.db.prepare('SELECT value FROM settings WHERE key = ?').get(STATE_KEY) as { value?: string } | undefined
  ...
  protected async runCheck(): Promise<void> {
    ...
    if (!OutlookAuthService.getInstance().isLoggedIn()) return
  ...
  private broadcast(): void {
    ...
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.isDestroyed()) {
        try { win.webContents.send(IPC_CHANNELS.CALENDAR_OUTLOOK_SYNC_CHANGED, status) } catch { /* ignore */ }
      }
    }
  }
  ...
  async runSync(): Promise<void> {
    ...
    try {
      ...
      logger.info(`Sync done: +${result.created} ~${result.updated} -${result.deleted} !${result.failed}`)
    } catch (err: any) {
  ...
  private async graph(token: string, method: string, path: string, body?: any): Promise<any> {
    ...
    if (resp.status === 401) {
      // token 失效，刷新后重试一次
      const fresh = await OutlookAuthService.getInstance().getAccessToken()
    ...
    if (!resp.ok) {
      ...
      logger.error(`Graph ${method} ${path} -> ${resp.status}: ${detail}`)
  ...
  private async ensureTargetCalendar(token: string): Promise<string> {
    ...
    logger.info(`Created Outlook calendar "${TARGET_NAME}"`)
    return created.id
  }
  ...
  private async ensureTodoList(token: string): Promise<string> {
    ...
    logger.info(`Created To Do list "${TARGET_NAME}"`)
    return created.id
  }
