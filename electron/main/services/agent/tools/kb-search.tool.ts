...
export function createKBSearchTool(allowedKbIds: string[]): ToolDefinition {
  ...
  return {
    ...
    name: 'kb_search',
    title: '智能知识库检索',
    description: `对知识库进行全方位智能检索，同时搜索文档标题、摘要、章节、关键词、实体和原始内容，返回最相关的结果。这是知识库检索的首选工具，支持多维度混合排序。

【使用场景】
- 当用户提出知识库相关问题但不确定具体检索维度时
- 需要快速了解知识库中与某主题相关的所有信息时
- 作为其他知识库工具的补充，获取更全面的检索结果
