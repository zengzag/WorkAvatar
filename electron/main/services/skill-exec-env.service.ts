...
import { IPC_CHANNELS, type SkillEnvTool, type SkillEnvToolId, type SkillEnvInstallProgress } from '../../shared/ipc-channels'
...
const DETECT_CONFIGS: Record<SkillEnvToolId, DetectConfig> = {
  uv: {
    commands: ['uv'],
    versionArgs: ['--version'],
    knownLocations: () => {
 ...
 */
async function detectTool(toolId: SkillEnvToolId): Promise<{ installed: boolean; version?: string; path?: string }> {
 ...
 */
function getToolMeta(toolId: SkillEnvToolId): { name: string; description: string; installable: boolean; installHint?: string } {
  switch (toolId) {
    case 'uv':
      return {
        name: 'uv',
        description: 'Python 包管理器，可一键安装并管理 Python 解释器，是 Skill 运行 Python 脚本的首选依赖',
        installable: true,
      }
    case 'python':
      return {
        name: 'Python',
        description: '运行 Skill 中的 Python 脚本（.py 文件）',
        installable: true,
    ...
    case 'node':
      return {
        name: 'Node.js',
        description: '运行 Skill 中的 JavaScript / TypeScript 脚本，并提供 npm 包管理',
    ...
    case 'pip':
      return {
        name: 'pip',
        description: 'Python 包管理工具，随 Python 一同安装，用于安装 Skill 所需的 Python 依赖包',
        installable: true,
...
const TOOL_ORDER: SkillEnvToolId[] = ['uv', 'python', 'node', 'pip']

class SkillExecEnvService {
  private static instance: SkillExecEnvService
  /** 当前正在安装的工具（同一时刻只允许一个） */
  private installingTool: SkillEnvToolId | null = null
  /** 当前安装进程的子进程引用，用于取消 */
   ...
   */
  async detectAll(): Promise<SkillEnvTool[]> {
   ...
   */
  async install(toolId: SkillEnvToolId): Promise<{ success: boolean; error?: string }> {
  ...
  private emitProgress(progress: SkillEnvInstallProgress): void {
    try {
      const windows = BrowserWindow.getAllWindows()
      for (const win of windows) {
        if (!win.isDestroyed()) {
          win.webContents.send(IPC_CHANNELS.SKILL_ENV_PROGRESS, progress)
        }
      }
    } catch (err) {
      logger.warn('Failed to emit skill env progress:', err)
    }
  }
  ...
  private runInstallCommand(
    command: string,
    args: string[],
    opts: { phase: SkillEnvInstallProgress['phase']; message: string; timeout?: number; env?: NodeJS.ProcessEnv }
  ...
  private async installPython(): Promise<{ success: boolean; error?: string }> {
    ...
    if (!detected.installed) {
      ...
      this.emitProgress({ toolId: 'python', phase: 'done', message: 'Python 已通过 uv 安装（uv 管理的 Python 不在系统 PATH 中，Skill 运行时会自动通过 uv 解析）' })
      return { success: true }
    }
