...
#### 智能员工画像分析
- 基于知识库内容自动分析业务场景
- 智能生成员工角色名称和详细描述（使用通用化角色名，避免过于具体的角色定位）
- 自动识别员工职责清单
- 推荐合适的工作风格和性格特质
- 支持用户补充说明和期望，引导分析方向
- 分析提示词根据知识库的业务领域、文档类型、关键实体和核心主题进行针对性分析
...
- 支持手动启用/禁用技能，调整技能参数
- 建议工具基于已有系统工具列表智能推荐，自动匹配开启

#### 创建流程
1. **基础配置**：选择关联知识库、LLM 模型、业务场景描述（可选）
2. **确认创建**：系统自动分析生成员工画像，用户可编辑修改；工具默认选中知识库检索和文件操作；工具设置（原高级配置）支持根据员工职责选择工具
3. **快速创建**：支持快速创建默认员工，无需 LLM 分析，直接使用默认配置生成
4. **工作区目录**：创建员工时自动创建专属工作区目录，支持手动更改
...
#### 系统提示词可配置
- 在员工设置中可查看当前自动生成的系统提示词完整内容
- 支持自定义覆盖系统提示词，根据需要调整员工的行为方式、角色身份和工作风格
- 自动生成提示词基于员工画像（角色名称、描述、职责、特质、工作风格等）构建
- 可在「系统提示词」选项卡中编辑和恢复默认提示词
